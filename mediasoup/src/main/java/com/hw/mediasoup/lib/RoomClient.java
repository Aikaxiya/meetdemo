...
import com.hw.mediasoup.lib.lv.SupplierMutableLiveData;
import com.hw.mediasoup.lib.model.Producers;
import com.hw.mediasoup.lib.socket.WebSocketTransport;
...
import static com.hw.mediasoup.lib.JsonUtils.jsonPut;
import static com.hw.mediasoup.lib.JsonUtils.toJsonObject;
...
public class RoomClient extends RoomMessageHandler {
    ...
    private void disposeTransportDevice() {
        ...
        // Close mediasoup Transports. 必须先关后销毁不然会发生AudioTrackJavaThread线程错误
        if (mSendTransport != null) {
    ...
    private void joinImpl() {
        ...
        try {
            ...
            //mMediasoupDevice.load加options 解决armeabi-v7a版本音频出错问题
            ...
            // 创建用于发送的 mediasoup 传输（除非我们不想生成）。
            if (mOptions.isProduce()) {
                createSendTransport();
            }

            // 创建用于发送的 mediasoup 传输（除非我们不想消费）。
    ...
    private void onNewConsumer(Message.Request request, Protoo.ServerRequestHandler handler) {
        ...
        try {
            ...
            Log.e(TAG, "onNewConsumer: " + data.toString());
            ...
            //家属会见监控端特殊处理
            if (peerId.startsWith("monitor_")) {
                consumer.getTrack().setEnabled(false);
            }
            ...
            // If audio-only mode is enabled, pause it. 如果启用了纯音频模式，请将其暂停。
    ...
    @Async
    public void screenSharing(MediaStreamTrack mediaStreamTrack) {
        Logger.d(TAG, "screenSharing(mediaStreamTrack)");
        mStore.setShareInProgress(true);
        mWorkHandler.post(() -> {
            screenSharingImpl(mediaStreamTrack);
    ...
    public void stopScreenSharing() {
        Logger.d(TAG, "stopScreenSharing()");
        mWorkHandler.post(this::stopScreenSharingImpl);
    }

    @WorkerThread
    public void screenSharingImpl(MediaStreamTrack mediaStreamTrack) {
        Logger.d(TAG, "screenSharingImpl(mediaStreamTrack)");
        try {
            if (mShareProducer != null) {
                return;
            }
            if (!mMediasoupDevice.isLoaded()) {
                Logger.w(TAG, "screenSharing() | not loaded");
                return;
            }
            if (!mMediasoupDevice.canProduce("video")) {
                Logger.w(TAG, "screenSharing() | cannot produce video");
                return;
            }
            if (mSendTransport == null) {
                Logger.w(TAG, "screenSharing() | mSendTransport doesn't ready");
                return;
            }
            //关闭摄像头
            disableCam();
            mScreenShareTrack = ((VideoTrack) mediaStreamTrack);
            mShareProducer = mSendTransport.produce(producer -> {
                Logger.e(TAG, "onTransportClose(), camProducer");
                if (mShareProducer != null) {
                    mStore.removeProducer(mShareProducer.getId());
                    mShareProducer = null;
                }
            }, mScreenShareTrack, null, null);
    ...
    @WorkerThread
    public void stopScreenSharingImpl() {
        Logger.d(TAG, "stopScreenSharingImpl()");
        if (mShareProducer == null) {
            return;
        }
        mShareProducer.close();
        mStore.removeProducer(mShareProducer.getId());
        try {
            mProtoo.syncRequest("closeProducer", req -> jsonPut(req, "producerId", mShareProducer.getId()));
        } catch (ProtooException e) {
            e.printStackTrace();
            mStore.addNotify("error", "Error closing server-side screenShare Producer: " + e.getMessage());
        }
        mShareProducer = null;
    }
}
